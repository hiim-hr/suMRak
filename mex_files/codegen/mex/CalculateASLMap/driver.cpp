...
#include "CalculateASLMap_internal_types.h"
#include "CalculateASLMap_internal_types1.h"
#include "CalculateASLMap_internal_types11.h"
              ...
              real_T &output_stepsize, real_T &output_firstorderopt)
{
  ...
  if ((!hasFiniteBounds) || (minWidth < 0.0)) {
    ...
    sqrtGamma = 0.0;
    for (k = 0; k < 2; k++) {
      absx = muDoubleScalarAbs(x[k] - 1.0);
  ...
  for (b_i = 0; b_i <= m; b_i++) {
    if (b_i + 1 > f_temp.size(1)) {
      emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, f_temp.size(1), &q_emlrtBCI,
                                    (emlrtConstCTX)&sp);
    }
    if (b_i + 1 > fCurrent.size(1)) {
      emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, fCurrent.size(1), &q_emlrtBCI,
                                    (emlrtConstCTX)&sp);
    }
  ...
  while ((!exitg1) && (b_exitflag == -5)) {
    ...
    for (b_i = 0; b_i <= m; b_i++) {
      ...
      if (b_i + 1 > fNew.size(1)) {
        emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, fNew.size(1), &q_emlrtBCI,
                                      (emlrtConstCTX)&sp);
      }
      fNew[b_i] = f_temp[b_i];
    }
                     ...
                     const real_T x[2], ::coder::array<real_T, 1U> &varargout_2)
{
  ...
  CalculateASLMap_anonFcn1(d_st, x, fun.workspace.xdata, F_temp);
