                 ...
                 real_T x[2])
{
  ...
  st.tls = sp.tls;
  emlrtHeapReferenceStackEnterFcnR2012b((emlrtConstCTX)&sp);
  r.set_size(&y_emlrtRTEI, &sp, 1, xdata.size(1));
  ...
  r.set_size(&y_emlrtRTEI, &sp, 1, ydata.size(1));
  ...
  b_this.workspace.xdata.set_size(&ab_emlrtRTEI, &sp, 1, xdata.size(1));
  loop_ub = xdata.size(1);
  for (int32_T i{0}; i < loop_ub; i++) {
    b_this.workspace.xdata[i] = xdata[i];
  }
  b_this.workspace.ydata.set_size(&ab_emlrtRTEI, &sp, 1, ydata.size(1));
  loop_ub = ydata.size(1);
  for (int32_T i{0}; i < loop_ub; i++) {
                          ...
                          ::coder::array<real_T, 2U> &varargout_1)
{
  ...
  CalculateASLMap_anonFcn1(b_st, x, xdata, varargout_1);
