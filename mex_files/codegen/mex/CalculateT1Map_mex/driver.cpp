...
#include "CalculateT1Map_mex_internal_types1.h"
                ...
                real_T &output_firstorderopt)
{
  ...
  if ((!hasFiniteBounds) || (minWidth < 0.0)) {
    ...
    x[1] = 0.03;
    if (hasUB[1]) {
      d = muDoubleScalarMin(ub[1], 0.03);
      x[1] = d;
    }
    absx = muDoubleScalarAbs(d - 0.03);
    if (absx > relFactor) {
      relFactor = absx;
    }
    if (relFactor > 0.0) {
  ...
  st.site = &y_emlrtRSI;
  b_st.site = &v_emlrtRSI;
  c_lsqcurvefit_anonFcn1(b_st, fun.workspace.xdata, fun.workspace.ydata, x,
                         f_temp);
  m_temp = f_temp.size(1);
  jacobian.set_size(&e_emlrtRTEI, &sp, f_temp.size(1), 2);
  m = f_temp.size(1);
  ...
  b_this.workspace.fun = fun;
  st.site = &y_emlrtRSI;
  utils::FiniteDifferences::c_factoryConstruct(st, b_this, f_temp.size(1), ub,
                                               FiniteDifferences);
  st.site = &y_emlrtRSI;
  funcCount = e_jacobianFiniteDifference(st, augJacobian, fCurrent, x, ub,
                                         FiniteDifferences, evalOK);
  b_gamma = 0.01;
  st.site = &y_emlrtRSI;
  augJacobian[f_temp.size(1)] = 0.0;
  ...
  while ((!exitg1) && (b_exitflag == -5)) {
    ...
    st.site = &y_emlrtRSI;
    b_st.site = &v_emlrtRSI;
    c_lsqcurvefit_anonFcn1(b_st, fun.workspace.xdata, fun.workspace.ydata, xp,
                           f_temp);
    st.site = &y_emlrtRSI;
    if (m_temp > 2147483646) {
      b_st.site = &p_emlrtRSI;
    ...
    if ((absx < resnorm) && evalOK) {
      ...
      st.site = &y_emlrtRSI;
      evalOK = f_jacobianFiniteDifference(st, augJacobian, fCurrent, funcCount,
                                          xp, ub, FiniteDifferences);
      st.site = &y_emlrtRSI;
      LAPACKE_dlacpy_work(102, 'A', (ptrdiff_t)m_temp, (ptrdiff_t)2,
                          &(augJacobian.data())[0], (ptrdiff_t)(m_temp + 2),
                       ...
                       ::coder::array<real_T, 1U> &varargout_2)
{
  ...
  c_st.site = &fc_emlrtRSI;
  d_st.site = &gc_emlrtRSI;
  F_temp.set_size(&mb_emlrtRTEI, &d_st, 1, fun.workspace.xdata.size(1));
  b_x = x[1];
